...
!pygame.init()
pygame.display.set_caption("AoC 2022 Pathfinding Day 12")
cell_font = pygame.font.SysFont(pygame.font.get_default_font(), 15)
...
def main_loop(ui):
    screen = pygame.display.set_mode((2000, 1600), pygame.RESIZABLE)
...
def draw_board(surface, area, board):
    ...
    for y in range(0, metrics.num_y):
        for x in range(0, metrics.num_x):
            cell = board.at([x, y])
            clr = colors.get(cell.type, (80, 80, 0))
            cell_rect = metrics.cell_rect([x, y])

            pygame.draw.rect(surface, clr, cell_rect)

            if cell.count != math.inf:
                #number = cell_font.render( "{} {}".format(cell.count,cell.weight), True, (255,255,255))
                number = cell_font.render("{}".format(
                    cell.count), True, (255, 255, 255))
...
def draw_path(surface, area, board, path):
    ...
    for i in range(0, len(path)-1):
        ...
        # print(ctr_a,ctr_b)
        pygame.draw.line(surface, (120, 220, 0),
                         ctr_a, ctr_b, metrics.spacing)
